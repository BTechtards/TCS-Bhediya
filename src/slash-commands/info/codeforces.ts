...
const pendingValidation: Map<string, { username: string; timestamp: number; problemId: string }> = new Map();
...
const getRandomProblem = async (): Promise<{ problemId: string; problemLink: string } | null> => {
...
const checkCompilationError = async (username: string, problemId: string, afterTimestamp: number): Promise<boolean> => {
...
const linkCodeforcesCommand: SlashCommand = {
  builder: new SlashCommandBuilder()
    ...
    .setDescription('Links or shows your Codeforces username linked to your Discord account.')
  ...
  chatCommandHandler: async (interaction) => {
    ...
    if (!interaction.options.getString('username')) {
      if (links[discordId]) {
        await interaction.reply(`You already have your Codeforces account linked to: **${links[discordId]}**`);
      } else {
        await interaction.reply("You haven't linked your Codeforces account yet. Please provide your username with this command.");
      }
      return;
    }
    ...
    const existingUser = Object.entries(links).find(([_, username]) => username === codeforcesUsername);
    if (existingUser && existingUser[0] !== discordId) {
      await interaction.reply(`The Codeforces username **${codeforcesUsername}** is already linked to another Discord account.`);
      return;
    }

    const isValidUsername = await validateCodeforcesUsername(codeforcesUsername);
    if (!isValidUsername) {
      await interaction.reply('Invalid Codeforces username. Please provide a valid username.');
      return;
    }

    const problemData = await getRandomProblem();
    if (!problemData) {
      await interaction.reply("Failed to fetch a valid Codeforces problem. Please try again.");
      return;
    }
    ...
    await interaction.reply(`To link your Codeforces account, submit a **compilation error** to the following problem within **45 seconds**:\n\n**Problem:** [${problemId}](${problemLink})\nOnce you submit the error, your account will be linked.`);

    pendingValidation.set(discordId, { username: codeforcesUsername, timestamp: commandTimestamp, problemId });
    ...
    setTimeout(async () => {
      ...
      const isCompilationError = await checkCompilationError(username, problemId, timestamp);
      
      if (isCompilationError) {
        links[discordId] = username;
        saveLinks(links);
        pendingValidation.delete(discordId);
        await interaction.followUp(`✅ Your Discord account has been successfully linked to **${username}**.`);
      } else {
        await interaction.followUp(`❌ You failed to submit a compilation error within the 45 seconds time limit. Please try again.`);
        pendingValidation.delete(discordId);
      }
    }, 45000);
  },
