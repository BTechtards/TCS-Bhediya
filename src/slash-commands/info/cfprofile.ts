...
function countSolvedProblems(submissions: any[]): number {
    let alreadySolved = new Set<string>();
    for (const submission of submissions) {
        if (submission.verdict === "OK") {
            alreadySolved.add(`${submission.problem.contestId}-${submission.problem.index}`);
        }
    }
    ...
    chatCommandHandler: async (interaction) => {
        ...
        if (!username) {
            await interaction.reply({
                content: "You haven't linked your Codeforces account yet. Please link your account first.",
                ephemeral: true,
            });
            return;
        }
        ...
        try {
            ...
            if (userInfoRes.status === "OK" && userStatusRes.status === "OK" && userRatingRes.status === "OK") {
                ...
                const embed = {
                    ...
                    fields: [
                        { name: "Member Since", value: new Date(user.registrationTimeSeconds * 1000).toLocaleDateString(), inline: true },
                        { name: "Current Ranking", value: formattedRank, inline: true },
                        { name: "Highest Rating", value: `${user.maxRating}`, inline: true },
                        { name: "Current Rating", value: `${user.rating}`, inline: true },
                        { name: "Problems Solved", value: `${problemsSolved}`, inline: true },
                        { name: "Contests Participated", value: `${contestsParticipated}`, inline: true },
                    ],
                    color: 0x88C0D0,
